...
public class VerticalTextPicker extends View {
    ...
    public void setItems(String[] textList) {
        mTextList = textList;
        Log.d("deqin",mTextList.length+"");
        calculateTextPositions();
    }

    private void calculateItem() {
        //Log.d("calculate item", getClass());
        mTextPaintBig.setTextSize(TEXT_SIZE);
        Rect item = new Rect();
       // Log.d("mTextList[0]->" + mTextList[0], getClass());
        mTextPaintBig.getTextBounds(mTextList[0], 0, mTextList[0].length(), item);
        text_height = item.height();
      //  Log.d("textHeight->" + text_height, getClass());
        text_width = item.width();
      //  Log.d("text_width->" + text_width, getClass());
        float real_text_item_height = (picker_text_canvas_height - text_margin * 2f) / 3f;
       // Log.d("real_text_item_height->" + real_text_item_height, getClass());
        float real_text_item_width = getMeasuredWidth() - HORIZONTAL_BACKGROUND_MARGIN * 2;
       // Log.d("real_text_item_width->" + real_text_item_width, getClass());
       // Log.d("getMeasuredWidth->" + getMeasuredWidth(), getClass());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //Log.d("widthsize->" + widthsize, getClass());
       // Log.d("widthMode->" + widthMode, getClass());
...
/*            case MeasureSpec.AT_MOST:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mWrapAround) {
            for (int i = 1; i <= mAmountOfItems / 2; i++) {
                mTextPaintLight.setTextSize(TEXT_SIZE-15*i);
                mTextPaintLight.setAlpha(mTextPaintBig.getAlpha()-50*i);
                if(i==1){

                    drawText(canvas, mTextPaintLight, center_baseline - SCROLL_DISTANCE * 4 / 3
                            + mTotalAnimatedDistance, mText[2 - i]);
                    drawText(canvas, mTextPaintLight, center_baseline + SCROLL_DISTANCE
                            + mTotalAnimatedDistance, mText[2 + i]);
                }

                if (i==mAmountOfItems/2){
                    drawText(canvas, mTextPaintLight, center_baseline -  SCROLL_DISTANCE * 4 / 3 - SCROLL_DISTANCE *3 / 4
                            + mTotalAnimatedDistance, mText[2 - i]);
                    drawText(canvas, mTextPaintLight, center_baseline + SCROLL_DISTANCE  + SCROLL_DISTANCE * 9 /16
                            + mTotalAnimatedDistance, mText[2 + i]);
                }
            }
            Paint paint = new Paint();
            paint.setColor(getResources().getColor(R.color.alarm_timepick_line));
            paint.setStrokeWidth(dpToPx(3));
            canvas.drawLine(center_x-text_width/2-dpToPx(8), center_y-text_height ,
                     center_x+text_width/2+dpToPx(8), center_y-text_height , paint);
            drawText(canvas, mTextPaintBig, center_baseline + mTotalAnimatedDistance, mText[2]);
            canvas.drawLine(center_x-text_width/2-dpToPx(8), center_y+text_height ,
                     center_x+text_width/2+dpToPx(8), center_y+text_height , paint);
        } else{
            Paint paint = new Paint();
            paint.setColor(getResources().getColor(R.color.alarm_timepick_line));
            paint.setStrokeWidth(dpToPx(3));
            canvas.drawLine(center_x-text_width/2-dpToPx(5), center_y-text_height ,
                    center_x+text_width/2+dpToPx(5), center_y-text_height , paint);
    ...
    private void calculateTextPositions() {
        if (mWrapAround) {
            mText[0] = getTextToDraw(-2);
            mText[1] = getTextToDraw(-1);
            mText[2] = getTextToDraw(0);
            mText[3] = getTextToDraw(1);
            mText[4] = getTextToDraw(2);
