...
public class UpdateService extends RemoteViewsService {
    ...
    class ListRemoteViewsFactory implements
        ...
        private void initTime(RemoteViews rv, TimeZoneInfo timeZone,
            ...
            rv.setTextViewText(idDate,
                    shortDateFormat.format(calendar.getTime()));
            ...
            if (!is24Hour) {
                ...
                if (!isMorning) {
                    if ((start13_hour <= dhs && dhs <= end13_hour)) {
                        rv.setImageViewResource(idDayNightIcon,
                                R.drawable.widget_worldclock_night);
                    } else {
                        rv.setImageViewResource(idDayNightIcon,
                                R.drawable.widget_worldclock_day);
                    }
                    rv.setTextViewText(idAmOrPm, getText(R.string.time_pm));
                } else {
                    if ((start13_hour <= dhs && dhs <= end13_hour)) {
                        rv.setImageViewResource(idDayNightIcon,
                                R.drawable.widget_worldclock_day);
                    ...
                    rv.setTextViewText(idAmOrPm, getText(R.string.time_am));
                }
            } else {
