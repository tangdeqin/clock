...
//		}
    ...
    */
	...
	protected void onStart() {
		...
		super.onStart();
		// PR856818 by xing.zhao [CTS Verifier 4.4_r3] Alarms and Timers Tests
		// #com.android.cts.verifier.deskclock.DeskClockTestsActivity begin
		final boolean skipUi = getIntent().getBooleanExtra(EXTRA_SKIP_UI, false);
		final int second = getIntent().getIntExtra(EXTRA_LENGTH, 0);
		String message = getIntent().getStringExtra(EXTRA_MESSAGE);
		Log.e("huihui2","intent "+getIntent()+skipUi+second+message+getIntent().getAction());
		if (ACTION_SET_TIMER.equals(getIntent().getAction()) && !skipUi) {
			if (second != 0) {
				Intent startIntent = new Intent(this, CountDownActivity.class);
				startIntent.putExtra("CountDownSecend", 30);
				startActivity(startIntent);
				finish();
			} else {
				mTabHost.setCurrentTabByTag(TIMER_TAB_TAG);
			}
		} else if (ACTION_SET_TIMER.equals(getIntent().getAction()) && skipUi) {
			Toast.makeText(this, message, Toast.LENGTH_SHORT).show();
			Intent timerServiceIntent = new Intent(this,
					BackgroundCountDownService.class);
			timerServiceIntent.putExtra(
					BackgroundCountDownService.TIMER_COUNT_EXTRA, second);
			timerServiceIntent.putExtra("CountDownTitleShow", message);
