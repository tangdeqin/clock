...
public class AlarmKlaxon extends Service {
    ...
    private void resetAlarmVolume() {
        if (needCaringMode() && !mCurrentAlarm.silent && mMaxVolume > 0) {
    ...
    private void play(Alarm alarm) {
        ...
        if (!alarm.silent && mMaxVolume > 0) {
            ...
            if (!file.exists()) {
                ...
                Uri defaultUri = RingtoneManager.getDefaultRingtoneUri(this,
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        try {
                            if (DrmManagerClient.isMTKDrmEnable()
                                    && DrmManagerClient.isDrm(alertPath)) {
                ...
                try {
                    if (DrmManagerClient.isMTKDrmEnable() && DrmManagerClient.isDrm(alertPath)) {
                        if (!isValidDrmRingtone(alertPath)) {
                            if (RingtoneList.getRingtoneList(this) != null
                                    && RingtoneList.getRingtoneList(this).length > 1) {
                                alert = RingtoneList.getRingtoneList(this)[1];
    ...
    private boolean isValidDrmRingtone(String filePath) {
        DrmManagerClient drmManager = DrmManagerClient.getInstance(this);
